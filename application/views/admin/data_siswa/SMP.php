...
<head>
  ...
  <title>Galileo Gasing | Data Siswa SMP</title>
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li><a href="<?php echo site_url('admin/datasiswa/SMP') ?>">Data Siswa</a></li>
        <li class="active">SMP</li>
      </ol>
    </section>
        ...
        <div class="col-xs-12">
          <div class="box">
            ...
            <div class="box-header">
              <h3 class="box-title"><i class="fa fa-users" aria-hidden="true"></i>  Total Siswa : <?php echo count($data_siswa);?></h3>
            </div>
            <br>
            <!-- /.box-header -->
            <div class="box-body table-responsive" style="overflow: auto;">
              <table id="example1" class="table table-bordered table-striped bg-table">
                ...
                <tr>
                  ...
                  <th>Foto</th>
                  <th>Operasi</th>
                  <th>Tetapkan tanggal jatuh tempo</th>
                  <th>Verifikasi</th>
                </tr>
                </thead>
                <tbody>
                  <?php foreach($data_siswa as $post){?>
                    <tr>
                       <td><?php echo $post->id_siswa?></td>
                       <td><?php echo $post->nama_depan." ".$post->nama_belakang?></td>
                       <td><?php echo $post->tempat_lahir?></td>
                       <td><?php echo $post->tanggal_lahir?></td>
                       <td><?php echo $post->username?></td>
                       <td><?php echo $post->email?></td>
                       <td><?php echo $post->jenis_kelamin?></td>
                       <td><?php echo $post->kelas." ".$post->level." ".$post->jurusan;?></td>
                       <td><?php echo $post->alamat?></td>
                       <td><?php echo $post->sekolah?></td>
                       <td><?php echo $post->telepon?></td>
                       <td><?php echo $post->tanggal_daftar?></td>
                       <td><?php echo $post->tanggal_jatuh_tempo?></td>
                       <td><img src = "<?php echo base_url(); ?><?php echo $post->foto?>" id="user-pic" alt="Student image"></td>
                       <td><?php echo anchor('admin/datasiswa/hapus_SMP/'.$post->id_siswa,'<i class="ion-trash-a"></i> Hapus'); ?></td>
                       <?php if($post->tanggal_jatuh_tempo == '0000-00-00'){ ?>
                        <td><form role="form" method="POST" action="<?php echo base_url('admin/datasiswa/tempo_SMP/'.$post->id_siswa); ?>"><input type="date" name="exp_date"><br><br><input type="submit" class="btn btn-danger" name="submit_exp" value="Ubah"></form></td>
                        <?php } else{ ?>
                        <td></td>
                        <?php } ?>
                        <?php if($post->status == 'BELUM'){ ?>
                         <td><form role="form" method="POST" action="<?php echo base_url('admin/datasiswa/verif_SMP/'.$post->id_siswa); ?>"><input type="submit" class="btn btn-danger" name="submit_verif" value="Verifikasi"></form></td>
                         <?php } else {?>
                         <td></td>
                        <?php } ?>
                    </tr>    
